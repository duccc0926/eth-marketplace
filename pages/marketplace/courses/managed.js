...
export default function ManagedCourses() {
    ...
    const activateCourse = async courseHash => {
        changeCourseState(courseHash, "activeCourse")
    }

    const deactivateCourse = async courseHash => {
        changeCourseState(courseHash, "deactiveCourse")
    }

    const changeCourseState = async (courseHash, method) => {
        try {
            await contract.methods
            [method](courseHash).send({
                from: account.data
            })
        } catch (e) {
    ...
    return (
        <>
            <MarketHeader />
            <CourseFilter onFilterSelect={(value) => setFilters({ state: value })}
